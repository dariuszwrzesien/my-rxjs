...
export class CartService {
  ...
  cartSubtotal = computed(() => this.cartItems().reduce((acc, item) => acc + (item.product.price * item.quantity), 0));
  deliveryFee = computed(() => this.cartSubtotal() > this.freeDeliveryThreshold ? 0 : this.deliveryCost);
  ...
  updateQuantity(cartItem: CartItem, quantity: number) {
    this.cartItems.update(items => items.map(item => item === cartItem ? { ...item, quantity } : item));
  }
